/* ===========================================================
   Penilaian Lapangan - Form Configuration
   Konfigurasi form khusus untuk penilaian lapangan
   =========================================================== */

(function(){
    // Definisi skor maksimal per bagian
    ...
    var PASSING_CRITERIA = {
        'pratama': 110.5,  // Min skor Pratama
        'madya': 167.5,    // Min skor Madya
        'mandiri': 232.5   // Min skor Mandiri
    };
  ...
  items: [
   {
    ...
    window.LAPANGAN_CONFIG = {
        sections: FORM_CONFIG,
        maxScores: SECTION_MAX_SCORES,
        passingCriteria: PASSING_CRITERIA,
        categoryRules: CATEGORY_RULES
    };

    // ==============  B. RENDER & LOGIC  =================
  // Export FORM_CONFIG to window object
  window.FORM_CONFIG = FORM_CONFIG;

  var form = document.getElementById('formLapangan');
  var dotsWrap = document.getElementById('stepDots');
  var totalSkorEl = document.getElementById('LapanganTotalScore');
  var sectionSkorEl = document.getElementById('LapanganSectionScore');
  var kategoriOutEl = document.getElementById('LapanganStatus');
...
function setupFileUpload() {
  ...
  function fieldHTML(f) {
    ...
    if (f.type === 'conditional-table') {
      ...
      html += '<tr>';
      html += '<td rowspan="2" class="border px-4 py-2 bg-gray-50" style="width: 250px">' + 
              'Jumlah sampah yang dikelola di TPS/TPS3R</td>';
      ...
      html += '<input type="number" name="' + f.no + '_organik" class="w-full p-2 text-center border rounded" ' +
              'min="0" placeholder="0">';
      ...
      html += '<td class="border px-4 py-2">';
      html += '<div class="flex items-center gap-2">';
      html += '<input type="number" name="' + f.no + '_anorganik" class="w-full p-2 text-center border rounded" ' +
              'min="0" placeholder="0">';
      html += '<span class="text-sm text-gray-500">kg/bulan</span>';
      html += '</div>';
      html += '</td>';
      ...
      html += '<p class="text-sm mt-2" style="color:#FF0000">' +
              '<i>*Catatan wajib diisi apabila memilih poin c dan d</i></p>';

      html += '</div></div>';

      // Add event handlers for visibility control
      setTimeout(function() {
          if (f.dependsOn) {
              var tableDiv = document.getElementById(f.no);
              var radios = document.querySelectorAll('input[name="' + f.dependsOn + '"]');
              var inputs = tableDiv.querySelectorAll('input[type="number"]');
              
              radios.forEach(function(radio) {
                  radio.addEventListener('change', function() {
                      var showTable = f.showOn.includes(this.value);
                      tableDiv.classList.toggle('hidden', !showTable);
                      
                      // Toggle required state of inputs
                      inputs.forEach(function(input) {
                          input.required = showTable;
                      });
                  });
              });

              // Check initial state
              var checkedRadio = document.querySelector('input[name="' + f.dependsOn + '"]:checked');
              if (checkedRadio && f.showOn.includes(checkedRadio.value)) {
                  tableDiv.classList.remove('hidden');
                  inputs.forEach(input => input.required = true);
              }
          }

          // Store/restore values in localStorage
          var allInputs = document.querySelectorAll('input[name^="' + f.no + '_"]');
    ...
    if (f.type === 'rwChoice4') {
        ...
        // Add evidence fields for field assessment
        html += `
            <div class="mt-4 p-4 rounded-lg border border-gray-200 bg-gray-50">
                <h4 class="font-semibold text-gray-700 mb-3">Bukti Lapangan</h4>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Lokasi Pengamatan <span class="text-red-600">*</span></label>
                        <input type="text" name="${esc(f.no)}_location" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Upload Foto/Dokumentasi <span class="text-red-600">*</span></label>
                        <input type="file" name="${esc(f.no)}_evidence" accept="image/*" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100" required multiple>
                        <p class="mt-1 text-xs text-gray-500">Format: JPG, PNG. Maks 5 MB per file. Boleh upload lebih dari 1 foto.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Catatan Pengamatan</label>
                        <textarea name="${esc(f.no)}_notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" placeholder="Tambahkan catatan atau temuan saat pengamatan..."></textarea>
                    </div>
                </div>
            </div>
        `;
  ...
  if (f.type === 'table-lokasi') {
    ...
    f.rows.forEach(function(row) {
        ...
        html += '<input type="number" name="' + f.no + '_' + row.id + '" class="w-full input !p-2" min="0" required>'; // Add required
        html += '</td>';
        html += '</tr>';
    });
    html += '</tbody>';
    ...
    // Add event handlers for data persistence
    setTimeout(function() {
        const inputs = document.querySelectorAll('input[name^="' + f.no + '_"]');
        inputs.forEach(function(input) {
            input.addEventListener('change', function() {
            ...
            // Restore value from localStorage if exists
  ...
  if (f.type === 'table-input') {
    ...
    f.columns.forEach(function(col) {
      html += '<th class="border px-4 py-2 bg-gray-50" style="width: ' + (col.width || '120px') + '">' + 
              esc(col.label) + '</th>';
    });
    html += '</tr></thead>';
    ...
    setTimeout(function() {
        // Handle data sync between tables
        if (f.no === 'C-table3') {
        ...
        // Handle visibility toggling for C7-table
        if (f.no === 'C7-table') {
            var radioButtons = document.querySelectorAll('input[name^="C7_"]');
            var tableDiv = document.getElementById('C7-table');
            if (tableDiv && radioButtons) {
                ...
                radioButtons.forEach(function(radio) {
                    radio.addEventListener('change', function() {
                        var showTable = ['b', 'c', 'd'].includes(this.value);
                        tableDiv.classList.toggle('hidden', !showTable);
        ...
        // Handle visibility toggling for C4-table
        if (f.no === 'C4-table') {
            var tableDiv = document.getElementById('C4-table');
            if (tableDiv) {
                // Show table when any C4 radio is selected
                var c4Radios = document.querySelectorAll('input[name^="C4_"]');
                c4Radios.forEach(function(radio) {
                    radio.addEventListener('change', function() {
                        tableDiv.classList.remove('hidden');
                        var inputs = tableDiv.querySelectorAll('input');
                        inputs.forEach(input => input.required = true);
        ...
        // Store/restore values in localStorage for persistence
        var inputs = document.querySelectorAll('input[name^="' + f.no + '_"]');
        inputs.forEach(function(input) {
            input.addEventListener('change', function() {
                localStorage.setItem(input.name, input.value);
            });
...
if (f.type === 'table-input-2col') {
  ...
  if (f.type === 'table-wrapper') {
    ...
    // Keep track of tables requiring initialization
    const tablesToInit = [];
    
    // Render each child table
    f.items.forEach(function(item) {
        html += fieldHTML(item);
        if (['table-lokasi', 'table-input'].includes(item.type)) {
            tablesToInit.push(item);
        }
    });
    html += '</div>';
    
    // Initialize table interactions after render
    setTimeout(function() {
        tablesToInit.forEach(function(item) {
            const tableDiv = document.getElementById(item.no);
            if (!tableDiv) return;
            
            // Setup data sync for connected tables
            if (item.no === 'C-table3') {
                var inputs = tableDiv.querySelectorAll('input[name^="C-table3_row1"]');
            ...
            // Store/restore values in localStorage
            var allInputs = tableDiv.querySelectorAll('input');
            allInputs.forEach(function(input) {
                input.addEventListener('change', function() {
                    localStorage.setItem(input.name, input.value);
                });
  ...
  function setupStepper(){
    ...
    function renderDots(){
      ...
      for (var i=0; i<secs.length; i++){
        ...
        // Ubah class untuk styling baru
        d.className = 'step-label'; 
        d.setAttribute('role','listitem');
        if (i===idx) d.setAttribute('aria-current','step');
        // Tambahkan label abjad sesuai bab
        d.textContent = String.fromCharCode(65 + i); // 65 = 'A'
...
function recalcAll() {
    try {
        ...
        const sectionSkorEl = document.getElementById('LapanganSectionScore');
        if (!sectionSkorEl) {
            console.error('Section score element not found, initializing with 0');
            return;
        }
        ...
        // Final recalculation for current section
        recalcSection();
    } catch (error) {
    ...
    try {
        ...
        if (kategoriOutEl) {
            if (PASSING_CRITERIA.hasOwnProperty(menujuKategori)) {
                ...
                var result = `${displayKategori} (${isPassing ? 'LULUS' : 'TIDAK LULUS'})`;
...
function recalcSection(){
    try {
        ...
        const sectionSkorEl = document.getElementById('LapanganSectionScore');
        if (!sectionSkorEl) {
            console.error('Section score element not found');
            return;
        }
        ...
        if (window.LAPANGAN_CONFIG && window.LAPANGAN_CONFIG.sections && window.LAPANGAN_CONFIG.sections[active]){
            var section = window.LAPANGAN_CONFIG.sections[active];
        ...
        var componentLetter = currentSectionTitle.charAt(0);
...
function setupCheckboxRequired(){
  ...
  window.recalcSection = recalcSection;
