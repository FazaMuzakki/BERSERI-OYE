(function(){
    ...
    function initializeFormElements() {
        return {
            form: document.getElementById('formLapangan'),
            dotsWrap: document.getElementById('stepDots'),
            totalScoreEl: document.getElementById('lapanganTotalScore'),
            sectionScoreEl: document.getElementById('lapanganSectionScore'),
            statusEl: document.getElementById('lapanganStatus'),
    ...
    function validateSelectedAssessment() {
        ...
        try {
            ...
            if (!data.id || !data.nama_wilayah || !data.kecamatan || !data.kabkota) {
    ...
    function updatePMSummary(elements, data) {
        console.log('Updating PM Summary with data:', data);
        
        // Update nama wilayah
        const namaWilayahEl = document.getElementById('namaWilayah');
        if (namaWilayahEl) {
            namaWilayahEl.textContent = data.desa || '-';
        }

        // Update lokasi wilayah
        const lokasiWilayahEl = document.getElementById('lokasiWilayah');
        if (lokasiWilayahEl) {
            lokasiWilayahEl.textContent = `${data.kecamatan || '-'}, ${data.kabkota || '-'}`;
        }

        // Update kategori yang dituju
        const kategoriEl = document.getElementById('kategoriDituju');
        if (kategoriEl) {
            kategoriEl.textContent = data.pmCategory || '-';
        }

        // Update status penilaian mandiri
        const statusEl = document.getElementById('statusPenilaianMandiri');
        if (statusEl) {
            statusEl.textContent = data.status || 'Belum Dinilai';
        }

        // Update badge mandiri
        const badgeEl = document.getElementById('badgeMandiri');
        if (badgeEl) {
            const isValid = data.status === 'Selesai';
            badgeEl.className = `px-2 py-0.5 text-xs font-medium rounded-full ${isValid ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`;
            badgeEl.textContent = isValid ? 'Valid' : 'Menunggu Validasi';
        }

        // Update skor mandiri
        const skorEl = document.getElementById('skorMandiri');
        if (skorEl) {
            skorEl.textContent = `Skor: ${data.pmScore ? parseFloat(data.pmScore).toFixed(1) : '0'}`;
        }
    ...
    function handleFormSubmission() {
        ...
        const completedAssessments = JSON.parse(localStorage.getItem('completed_lapangan') || '[]');
        completedAssessments.push(assessmentData);
        localStorage.setItem('completed_lapangan', JSON.stringify(completedAssessments));
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('Initializing lapangan form...');
        
        // Verify summary section exists
        const summarySection = document.querySelector('#summarySection .p-3.rounded-lg.bg-white.border');
        if (!summarySection) {
            console.error('Summary section not found in HTML');
            return;
        }
        ...
        if (!window.FORM_CONFIG) {
            console.error('FORM_CONFIG not loaded. Ensure penilaian-lapangan-form-config.js is loaded first');
            return;
        }
        ...
        if (typeof render === 'function') {
            ...
            console.error('render function not found in penilaian-lapangan-form-static.js');
            return;
        }
